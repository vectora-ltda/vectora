...
class MediaQuota:
    ...
    def __init__(self, database: Path | None = None) -> None:
        self.database = database or settings.vectora_home / "media_quota.sqlite3"
    ...
    def _reserve(
        ...
        with self._connect() as db:
            ...
            if existing and existing[4] in {"reserved", "finalized"}:
    ...
    async def finalize(self, reservation: QuotaReservation, *, state: str) -> None:
        await asyncio.to_thread(self._finalize, reservation.id, state)

    def _finalize(self, reservation_id: str, state: str) -> None:
        with self._connect() as db:
            db.execute(
