...
function run(args: string[], cwd = process.cwd()): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = spawn("eslint", args, { cwd, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = "";
    let stderr = "";
    const timer = setTimeout(() => { child.kill("SIGKILL"); reject(new Error("ESLint timed out")); }, 15_000);
    ...
    child.once("close", (code) => { clearTimeout(timer); if (code !== 0 && !stdout) reject(new Error(stderr.trim() || `ESLint exited with code ${code ?? "unknown"}`)); else resolve(stdout || stderr); });
  });
}
...
export async function handle(request: Request): Promise<{ items: unknown[] }> {
  ...
  let items: unknown[];
  try { items = JSON.parse(output) as unknown[]; } catch { items = [{ output }]; }
  return { items };
}
