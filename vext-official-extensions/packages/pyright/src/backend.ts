...
function run(args: string[], cwd = process.cwd()): Promise<string> {
  return new Promise((resolve, reject) => {
    const child = spawn("pyright", args, { cwd, stdio: ["ignore", "pipe", "pipe"] });
    let stdout = ""; let stderr = "";
    const timer = setTimeout(() => { child.kill("SIGKILL"); reject(new Error("Pyright timed out")); }, 15_000);
    ...
    child.once("close", (code) => { clearTimeout(timer); if (code !== 0 && !stdout && args[0] !== "--version") reject(new Error(stderr.trim() || `Pyright exited with code ${code ?? "unknown"}`)); else resolve(stdout || stderr); });
  });
}
...
export async function handle(request: Request): Promise<{ items: unknown[] }> {
  ...
  const output = await run(["--outputjson", ...(request.params?.files ?? ["."])], cwd);
